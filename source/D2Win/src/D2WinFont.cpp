...
void __fastcall sub_6F8A9E90(int a1) {
	...
	D2FontStrc* pFont = (D2FontStrc*)ARCHIVE_ALLOC_BUFFER_AND_READ_FILE_TO_IT(D2Win_GetArchive(), szFilename, nullptr);
...
void __fastcall D2Win_10119_DrawCroppedText(const Unicode* wszText, int32_t nX, int32_t nY, int32_t a4, int32_t a5, int32_t a6, int32_t a7) {
	...
	while (nCharIdx < nTextLength) {
		...
		if (/*currentChar < 256 && */ currentChar == 0xFF) // currentChar is ASCII and equal �
		{
