...
int32_t __fastcall D2GAME_PLRMODES_First_6FC7F340(D2GameStrc* pGame, D2UnitStrc* pUnit, int32_t nMode, int32_t a4, D2SkillStrc* pSkill) {
	...
	if ((STATES_CheckState(pUnit, STATE_CONCENTRATION) && (ITEMS_RollRandomNumber(&pUnit->pSeed) % 100) < STATLIST_GetStatValue(STATLIST_GetStatListFromUnitAndState(pUnit, STATE_CONCENTRATION), STAT_SKILL_CONCENTRATION, 0)) || STATES_CheckState(pUnit, STATE_CONCENTRATE)) {
		if (pUnit->dwAnimMode == PLRMODE_NEUTRAL) {
			sub_6FC817D0(pGame, pUnit, nullptr, PLRMODE_NEUTRAL, 0, 0, 1);
			return 1;
		}
...
D2UnitStrc* __fastcall D2GAME_CORPSE_Handler_6FC7FBD0(D2GameStrc* pGame, D2UnitStrc* pUnit, int32_t nX, int32_t nY, D2ActiveRoomStrc* pRoom) {
	...
	for (int32_t nBodyLoc = -1; nBodyLoc < 13; ++nBodyLoc) {
		...
		if (pItem) {
			...
			D2UnitStrc* pDupedItem = ITEMS_Duplicate(pGame, pItem, pUnit, 1);
			if (nBodyLoc == -1) {
				if (INVENTORY_PlaceItemAtFreePosition(pDeadBody->pInventory, pDupedItem, UNITS_GetInventoryRecordId(pDeadBody, INVPAGE_INVENTORY, pGame->bExpansion), 0, 0, __FILE__, __LINE__) && INVENTORY_PlaceItemInSocket(pDeadBody->pInventory, pDupedItem, 1)) {
					if (pDupedItem) {
...
void __fastcall sub_6FC80E10(D2GameStrc* pGame, D2UnitStrc* pPlayer) {
	if (pPlayer && pPlayer->dwUnitType == UNIT_PLAYER) {
		...
		while (pItem) {
			if (INVENTORY_GetItemNodePage(pItem) == 3 && INVENTORY_UnitIsItem(pItem) && ITEMS_CheckItemTypeId(pItem, ITEMTYPE_WEAPON) && ITEMS_HasDurability(pItem) && (int32_t)STATLIST_UnitGetStatValue(pItem, STAT_DURABILITY, 0) <= 0 && !ITEMS_CheckItemFlag(pItem, 0x100u, __LINE__, __FILE__)) {
