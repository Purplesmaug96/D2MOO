...
void __fastcall PLRTRADE_HandleCubeInteraction(D2GameStrc* pGame, D2UnitStrc* pUnit) {
	...
	for (int32_t nId = 0; nId < nCubeMainTxtRecordCount; ++nId) {
		...
		if ((pCubeMainTxtRecord->nEnabled || dword_6FD4DC30) && (pGame->bExpansion || pCubeMainTxtRecord->wVersion < 100) && (pGame->nGameType || pGame->dwGameType || !pCubeMainTxtRecord->nLadder) && pCubeMainTxtRecord->nMinDiff <= pGame->nDifficulty && (pCubeMainTxtRecord->nClass == ((uint8_t)(-1)) || pCubeMainTxtRecord->nClass == pUnit->dwClassId) && pCubeMainTxtRecord->nNumInputs == nItemsInCube) {
