...
int32_t __fastcall D2GAME_PACKETCALLBACK_Rcv0x61_DropPickupMercItem_6FC88930(D2GameStrc* pGame, D2UnitStrc* pUnit, void* pPacket, int32_t nSize) {
	...
	if (pItem) {
		if (ITEMS_IsQuestItem(pItem) || !pGame->bExpansion || !ITEMS_CheckItemFlag(pItem, IFLAG_IDENTIFIED, __LINE__, __FILE__) || ITEMS_CheckItemFlag(pItem, IFLAG_BROKEN, __LINE__, __FILE__)) {
			return 0;
		}
