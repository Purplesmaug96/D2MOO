...
BOOL __stdcall GAME_CreateNewEmptyGame(char* szGameName, const char* szPassword, const char* szGameDescription, uint32_t nFlags, uint8_t nArenaTemplate, uint8_t nMaxLevelDifference, uint8_t nMaxPlayers, uint16_t* pGameId) {
	...
	*pHGame = D2GameReservedSlotHandle;
	...
	if (nInitSeed_6FDC2CA08 == -1) {
		...
		SEED_InitLowSeed(&pGame->pGameSeed, SEED_GetRandomValue(seed.u.LowPart));
...
_Requires_lock_held_(*pGame->lpCriticalSection)
	_Releases_lock_(*pGame->lpCriticalSection) void __fastcall
	GAME_FreeGame(D2GameGUID nGameGUID, D2GameStrc* pGame) {
	if (pGame) {
	...
	if (gpD2EventCallbackTable_6FD45830) {
		...
		case PACKET_ADMIN_GETGAMEINFO:
			if (gpD2EventCallbackTable_6FD45830->pfHandlePacket && gpD2EventCallbackTable_6FD45830->pfServerLogMessage) {
				...
				tPacket.tServerInfo.field_0xb4 = tGameServerInfoEx.field_0xb8 - InterlockedCompareExchange(&gnNumAdminConnections_6FD45838, 0, 0) + tGameServerInfoEx.dwords0xBC[0];
...
int32_t __stdcall GAME_UpdateGamesProgress(int32_t a1) {
	...
	start.u.HighPart = (uint64_t)(end.QuadPart - start.QuadPart) >> 32;
...
_Acquires_lock_(*(return->lpCriticalSection))
	_Acquires_lock_(return->lpCriticalSection)
		D2GameStrc* __fastcall
		GAME_LockGame(D2GameGUID nGameGUID) {
	if (!gpGameDataTbl_6FD45818) {
		return nullptr;
	}
