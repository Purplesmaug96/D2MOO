...
void __fastcall CLIENTS_FillCharacterPreviewInfo(D2ClientStrc* pClient, D2CharacterPreviewInfoStrc* pCharacterPreviewInfo) {
	...
	if (D2UnitStrc* pPlayer = CLIENTS_GetPlayerFromClient(pClient, FALSE)) {
		...
		if (!nPlayerLevel || nPlayerLevel > 99u || nPlayerClassId >= NUMBER_OF_PLAYERCLASSES || (tClientFlags.bInit || tClientFlags.bUnkFlag0x02 || tClientFlags.bUnkFlag0x10) || SStrLen((const char*)pCharacterPreviewInfo) != 33) // If any member of D2SaveLaunchStrc is 0. Looks like a debug check that is not required since we only fill with non-zero values.
		{
...
static bool IsWarpAttemptToBeConsideredForDelay(D2GameStrc* pGame, int32_t nLastWarpFrame) {
	return nLastWarpFrame != 0 && (uint32_t)(pGame->dwGameFrame - nLastWarpFrame) <= kWarpAttemptExpirationFrameCount;
}
