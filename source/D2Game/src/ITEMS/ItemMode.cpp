...
void __fastcall D2GAME_ITEMS_UpdateTransferredProperties_6FC424E0(D2GameStrc* pGame, D2UnitStrc* pItem, D2UnitStrc* pTarget, int32_t bUpdateStatList, int32_t bUpdateVitals) {
	...
	if (UNITS_CanDualWield(pTarget) && pWeapon && ITEMS_CheckItemTypeId(pItem, ITEMTYPE_WEAPON) && pWeapon != pItem && (!ITEMS_CheckItemFlag(pWeapon, IFLAG_BROKEN, __LINE__, __FILE__) || bUpdateStatList) && STATLIST_GetOwner(pWeapon, nullptr)) {
	...
	while (pCurrent) {
		...
		if (pCheckedItem) {
			...
			if (pItemsTxtRecord->nQuest && ITEMS_GetInvPage(pCheckedItem) != 1 && pItemsTxtRecord->nQuest == pItemsTxtRecordArg->nQuest) {
				...
				if (nCode1 == nCode2 || nCode1 == ' 43j' && nCode2 == ' 43g' || nCode1 == ' 43g' && nCode2 == ' 43j' || nCode1 == ' skb' && nCode2 == ' dkb' || nCode1 == ' dkb' && nCode2 == ' skb' || nCode1 == ' 33d' && nCode2 == ' 33g' || nCode1 == ' 33g' && nCode2 == ' 33d' || nCode1 == ' tsh' && nCode2 == ' fsm' || nCode1 == ' fsm' && nCode2 == ' tsh' || nCode1 == ' tsh' && nCode2 == ' piv' || nCode1 == ' piv' && nCode2 == ' tsh' || nCode1 == ' 2fq' && nCode2 == ' 1fq' || nCode1 == ' 1fq' && nCode2 == ' 2fq' || nCode1 == ' 2fq' && nCode2 == ' rhq' || nCode1 == ' rhq' && nCode2 == ' 2fq' || nCode1 == ' 2fq' && nCode2 == ' yeq' || nCode1 == ' yeq' && nCode2 == ' 2fq' || nCode1 == ' 2fq' && nCode2 == ' rbq' || nCode1 == ' rbq' && nCode2 == ' 2fq') {
					return 1;
				}
			}
		}
...
void __fastcall D2GAME_ITEMS_UpdateInventoryItems_6FC44A90(D2GameStrc* pGame, D2UnitStrc* pUnit, int32_t bRefresh) {
	...
	while (pItem) {
		if (INVENTORY_UnitIsItem(pItem) && INVENTORY_GetItemNodePage(pItem) == 1 && ITEMS_IsCharmUsable(pItem, pUnit) && !STATLIST_GetOwner(pItem, nullptr) && ITEMS_CheckRequirements(pItem, pUnit, 0, nullptr, nullptr, nullptr)) {
			int32_t bUpdate = 0;
	...
	for (int32_t i = 0; i < 11; ++i) {
		...
		if (pBodyLocItem && (!ITEMS_CheckItemFlag(pBodyLocItem, IFLAG_BROKEN, __LINE__, __FILE__) || STATLIST_GetOwner(pBodyLocItem, 0)) && (!ITEMS_CheckItemFlag(pBodyLocItem, IFLAG_NOEQUIP, __LINE__, __FILE__) || STATLIST_GetOwner(pBodyLocItem, 0))) {
			int32_t bUpdate = 0;
	...
	do {
		...
		for (int32_t i = 0; i < 11; ++i) {
			D2UnitStrc* pBodyLocItem = INVENTORY_GetItemFromBodyLoc(pUnit->pInventory, i);
			if (pBodyLocItem && !ITEMS_CheckItemFlag(pBodyLocItem, IFLAG_BROKEN, __LINE__, __FILE__) && (ITEMS_CheckItemFlag(pBodyLocItem, IFLAG_NOEQUIP, __LINE__, __FILE__) || !STATLIST_GetOwner(pBodyLocItem, 0)) && ITEMS_CheckRequirements(pBodyLocItem, pUnit, 0, nullptr, nullptr, nullptr)) {
				int32_t bUpdate = 0;
				if (!ITEMS_GetQuiverType(pBodyLocItem)) {
					bUpdate = 1;
				} else {
	...
	for (int32_t i = 0; i < 11; ++i) {
		D2UnitStrc* pBodyLocItem = INVENTORY_GetItemFromBodyLoc(pUnit->pInventory, i);
		if (pBodyLocItem && ITEMS_GetItemQuality(pBodyLocItem) == ITEMQUAL_SET && !ITEMS_CheckItemFlag(pBodyLocItem, IFLAG_BROKEN, __LINE__, __FILE__) && !ITEMS_CheckItemFlag(pBodyLocItem, IFLAG_NOEQUIP, __LINE__, __FILE__)) {
			D2GAME_ITEMS_UpdateTransferredProperties_6FC424E0(pGame, pBodyLocItem, pUnit, 1, 1);
			D2GAME_ITEMS_UpdateItemStatlist_6FC42310(pGame, pBodyLocItem, pUnit, 1);
		}
	}
...
int32_t __fastcall D2GAME_Transmogrify_6FC4A660(D2GameStrc* pGame, D2UnitStrc* pUnit, D2UnitStrc* pItem) {
	...
	if (!D2GAME_PlaceItem_6FC44410(__FILE__, __LINE__, pGame, pUnit, pNewItem->dwUnitId, itemDrop.nX, itemDrop.nY, 0, 1, 0) && !D2GAME_PlaceItem_6FC44410(__FILE__, __LINE__, pGame, pUnit, pNewItem->dwUnitId, 0, 0, 1, 1, 0)) {
