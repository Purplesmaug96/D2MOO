...
int32_t __fastcall SKILLS_SrvSt11_Inferno_ArcticBlast(D2GameStrc* pGame, D2UnitStrc* pUnit, int32_t nSkillId, int32_t nSkillLevel) {
	D2SkillsTxt* pSkillsTxtRecord = SKILLS_GetSkillsTxtRecord(nSkillId);
	if (pSkillsTxtRecord && (STATES_CheckState(pUnit, STATE_INFERNO) || STATLIST_UnitGetStatValue(pUnit, STAT_MANA, 0) >= pSkillsTxtRecord->wStartMana << 8) && pSkillsTxtRecord->wSrvMissileA >= 0 && pSkillsTxtRecord->wSrvMissileA < sgptDataTables->nMissilesTxtRecordCount) {
...
int32_t __fastcall SKILLS_SrvDo021_Telekinesis(D2GameStrc* pGame, D2UnitStrc* pUnit, int32_t nSkillId, int32_t nSkillLevel) {
	...
	case UNIT_ITEM: {
		if (ITEMS_CheckItemTypeId(pTarget, ITEMTYPE_SCROLL) || ITEMS_CheckItemTypeId(pTarget, ITEMTYPE_GOLD) || ITEMS_CheckItemTypeId(pTarget, ITEMTYPE_MISSILE_POTION) || ITEMS_CheckItemTypeId(pTarget, ITEMTYPE_MISSILE) || ITEMS_CheckItemTypeId(pTarget, ITEMTYPE_POTION) || ITEMS_CheckItemTypeId(pTarget, ITEMTYPE_KEY)) {
			int32_t a4 = 0;
