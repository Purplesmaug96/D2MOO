...
int32_t __fastcall ACT1Q5_UnitIterate_StatusCyclerEx(D2GameStrc* pGame, D2UnitStrc* pUnit, void* pData) {
	...
	if (!QUESTRECORD_GetQuestState(pQuestFlags, QUESTSTATEFLAG_A1Q5, QFLAG_REWARDGRANTED) || QUESTRECORD_GetQuestState(pQuestFlags, QUESTSTATEFLAG_A1Q5, QFLAG_PRIMARYGOALDONE) || QUESTRECORD_GetQuestState(pQuestFlags, QUESTSTATEFLAG_A1Q5, QFLAG_COMPLETEDNOW)) {
...
void __fastcall ACT1Q5_Callback11_ScrollMessage(D2QuestDataStrc* pQuestData, D2QuestArgStrc* pQuestArg) {
	...
	if ((nNpcId == MONSTER_CHARSI || nNpcId == MONSTER_KASHYA || nNpcId == MONSTER_CAIN5 || nNpcId == MONSTER_WARRIV1 || nNpcId == MONSTER_AKARA || nNpcId == MONSTER_GHEED) && (nMsgId == 144 || nMsgId == 140 || nMsgId == 145 || nMsgId == 141 || nMsgId == 143 || nMsgId == 142)) {
...
void __fastcall ACT1Q5_Callback13_PlayerStartedGame(D2QuestDataStrc* pQuestData, D2QuestArgStrc* pQuestArg) {
	...
	if (QUESTRECORD_GetQuestState(pQuestFlags, QUESTSTATEFLAG_A1Q5, QFLAG_ENTERAREA) == 1) { // NOLINT(bugprone-branch-clone)
		pQuestData->fState = 3;
