...
bool __fastcall ACT1Intro_ActiveFilterCallback(D2QuestDataStrc* pQuest, int32_t nNpcId, D2UnitStrc* pPlayer, D2BitBufferStrc* pQuestFlags, D2UnitStrc* pNPC) {
	return QUESTRECORD_GetQuestState(UNITS_GetPlayerData(pPlayer)->pQuestData[pQuest->pGame->nDifficulty], QUESTSTATEFLAG_A1Q1, QFLAG_REWARDGRANTED) != 1 && nNpcId == MONSTER_AKARA && !PLRINTRO_GetQuestIntroFlag(pPlayer, pQuest->pGame, MONSTER_AKARA);
}
