		...
		if (!bGameEnter) {
			if (byte_6FD31270) {
				for (int32_t i = 0; i < std::size(gpQuestInitTable); ++i) {
					...
					if (!gpQuestInitTable[i].bNoSetState && (QUESTRECORD_GetQuestState(pQuestFlags, gpQuestInitTable[i].nQuestNo, QFLAG_REWARDGRANTED) || QUESTRECORD_GetQuestState(pQuestFlags, gpQuestInitTable[i].nQuestNo, QFLAG_COMPLETEDBEFORE))) {
		...
		for (int32_t y = j; y > i; y -= 2) {
			...
			if (pRoom1) {
				for (int32_t x = i; x < j; x += 2) {
					...
					if (pRoom2 && !COLLISION_CheckMaskWithSizeXY(pRoom2, nX, nY, 2 * nSize + 1, 2 * nSize + 1, fCollision) && nX >= drlgCoords.nSubtileX && nX < drlgCoords.nSubtileX + drlgCoords.nSubtileWidth && nY >= drlgCoords.nSubtileY && nY < drlgCoords.nSubtileY + drlgCoords.nSubtileHeight) {
