...
void __fastcall OBJECTS_InitFunction46_TrappedSoulPlaceHolder(D2ObjInitFnStrc* pOp) {
	...
	for (int32_t nLoops = 12; ii > 0 && nLoops > 0; --nLoops) {
		...
		if (pRoomCoord.nSubtileWidth >= 5 && pRoomCoord.nSubtileHeight >= 5 && nX > pRoomCoord.nSubtileX + 1 && nY > pRoomCoord.nSubtileY + 1 && nX < pRoomCoord.nSubtileWidth + pRoomCoord.nSubtileX - 5 && nY < pRoomCoord.nSubtileY + pRoomCoord.nSubtileHeight - 5 && !COLLISION_CheckMaskWithSizeXY(pOp->pRoom, nX, nY, 10, 10, COLLIDE_WALL | COLLIDE_OBJECT | COLLIDE_DOOR) && !COLLISION_CheckMaskWithSizeXY(pOp->pRoom, nX, nY, 3, 3, COLLIDE_MASK_PLACEMENT)) {
			bPositionFound = 1;
			...
			while (1) {
				...
				while (nCounter < 3 * ii) {
					...
					++nCounter;
					if (pRoomCoord.nSubtileWidth >= 5 && pRoomCoord.nSubtileHeight >= 5 && nX > pRoomCoord.nSubtileX + 1 && nY > pRoomCoord.nSubtileY + 1 && nX < pRoomCoord.nSubtileX + pRoomCoord.nSubtileWidth - 5 && nY < pRoomCoord.nSubtileY + pRoomCoord.nSubtileHeight - 5 && !COLLISION_CheckMaskWithSizeXY(pOp->pRoom, nX, nY, 10, 10, COLLIDE_WALL | COLLIDE_OBJECT | COLLIDE_DOOR) && !COLLISION_CheckMaskWithSizeXY(pOp->pRoom, nX, nY, 3, 3, COLLIDE_MASK_PLACEMENT)) {
						bPositionFound = 1;
						break;
					}
				}
...
D2UnitStrc* __fastcall OBJECTS_PopulateFn1_CasketJarSarcophagusUrn(D2GameStrc* pGame, D2ActiveRoomStrc* pRoom, uint8_t nDensity, uint32_t nObjectId, uint16_t nProbability) {
	...
	while (nDensitya > 0 && v51 > 0) {
		...
		if (drlgCoords.nSubtileWidth >= pObjectsTxtRecord->dwSizeX + 2 && drlgCoords.nSubtileHeight >= pObjectsTxtRecord->dwSizeY + 2 && nX > drlgCoords.nSubtileX + 1 && nY > drlgCoords.nSubtileY + 1 && nX < drlgCoords.nSubtileX - pObjectsTxtRecord->dwSizeX + drlgCoords.nSubtileWidth - 2 && nY < drlgCoords.nSubtileY - pObjectsTxtRecord->dwSizeY + drlgCoords.nSubtileHeight - 2 && !COLLISION_CheckMaskWithSizeXY(pRoom, nX, nY, pObjectsTxtRecord->dwSizeX + 7, pObjectsTxtRecord->dwSizeY + 7, 0xC01u) && !COLLISION_CheckMaskWithSizeXY(pRoom, nX, nY, pObjectsTxtRecord->dwSizeX, pObjectsTxtRecord->dwSizeY, COLLIDE_MASK_PLACEMENT)) {
			D2_ASSERT(pRoom);
		...
		LABEL_45:
			if (!ITEMS_RollLimitedRandomNumber(&pObjectControl->pSeed, nSpawnedObjects >> 1) && v28) {
				...
				while (1) {
					...
					v28 = 0;
					if (v68) {
						if (drlgCoords.nSubtileWidth >= 2 && drlgCoords.nSubtileHeight >= 2 && nX > drlgCoords.nSubtileX + 2 && nY > drlgCoords.nSubtileY + 2 && nX < drlgCoords.nSubtileWidth + drlgCoords.nSubtileX - pObjectsTxtRecord->dwSizeX && nY < drlgCoords.nSubtileHeight + drlgCoords.nSubtileY - pObjectsTxtRecord->dwSizeY && !COLLISION_CheckMaskWithSizeXY(pRoom, nX, nY, pObjectsTxtRecord->dwSizeX + 2, pObjectsTxtRecord->dwSizeY + 2, COLLIDE_MASK_PLACEMENT)) {
							v28 = 1;
						}
					} else {
						if (drlgCoords.nSubtileWidth >= pObjectsTxtRecord->dwSizeX + 2 && drlgCoords.nSubtileHeight >= pObjectsTxtRecord->dwSizeY + 2 && nX > drlgCoords.nSubtileX + 1 && nY > drlgCoords.nSubtileY + 1 && nX < drlgCoords.nSubtileX - pObjectsTxtRecord->dwSizeX + drlgCoords.nSubtileWidth - 2 && nY < drlgCoords.nSubtileY - pObjectsTxtRecord->dwSizeY + drlgCoords.nSubtileHeight - 2 && !COLLISION_CheckMaskWithSizeXY(pRoom, nX, nY, pObjectsTxtRecord->dwSizeX + 7, pObjectsTxtRecord->dwSizeY + 7, 0xC01u) && !COLLISION_CheckMaskWithSizeXY(pRoom, nX, nY, pObjectsTxtRecord->dwSizeX, pObjectsTxtRecord->dwSizeY, COLLIDE_MASK_PLACEMENT)) {
...
D2UnitStrc* __fastcall OBJECTS_PopulateFn7_RogueOnStick(D2GameStrc* pGame, D2ActiveRoomStrc* pRoom, uint8_t nDensity, uint32_t nObjectId, uint16_t nProbability) {
	...
	for (int32_t i = 0; i < 8; ++i) {
		...
		if (drlgCoords.nSubtileWidth >= 7 && drlgCoords.nSubtileHeight >= 7 && (uint16_t)nBaseX > drlgCoords.nSubtileX + 1 && (uint16_t)nBaseY > drlgCoords.nSubtileY + 1 && (uint16_t)nBaseX < drlgCoords.nSubtileWidth + drlgCoords.nSubtileX - 7 && (uint16_t)nBaseY < drlgCoords.nSubtileHeight + drlgCoords.nSubtileY - 7 && !COLLISION_CheckMaskWithSizeXY(pRoom, nBaseX, nBaseY, 12u, 12u, 0xC01u) && !COLLISION_CheckMaskWithSizeXY(pRoom, nBaseX, nBaseY, 5u, 5u, COLLIDE_MASK_PLACEMENT)) {
			for (int32_t j = 0; j < stru_6FD2F820[nIndex].nCount; ++j) {
				...
				if (drlgCoords.nSubtileWidth >= (int32_t)(pObjectsTxtRecord->dwSizeX + 2) && drlgCoords.nSubtileHeight >= (int32_t)(pObjectsTxtRecord->dwSizeY + 2) && (uint16_t)nX > drlgCoords.nSubtileX + 1 && (uint16_t)nY > drlgCoords.nSubtileY + 1 && (uint16_t)nX < (int32_t)(drlgCoords.nSubtileX + drlgCoords.nSubtileWidth - pObjectsTxtRecord->dwSizeX - 2) && (uint16_t)nY < (int32_t)(drlgCoords.nSubtileY + drlgCoords.nSubtileHeight - pObjectsTxtRecord->dwSizeY - 2) && !COLLISION_CheckMaskWithSizeXY(pRoom, nX, nY, pObjectsTxtRecord->dwSizeX + 7, pObjectsTxtRecord->dwSizeY + 7, 0xC01u) && !COLLISION_CheckMaskWithSizeXY(pRoom, nX, nY, pObjectsTxtRecord->dwSizeX, pObjectsTxtRecord->dwSizeY, COLLIDE_MASK_PLACEMENT)) {
...
D2UnitStrc* __fastcall OBJECTS_CreateObject(D2GameStrc* pGame, int32_t nClassId, int32_t nSizeX, int32_t nSizeY, D2ActiveRoomStrc* pRoom) {
	...
	for (int32_t i = 0; i < 5; ++i) {
		...
		if (nX && nY && nX >= drlgCoords.nSubtileX + 1 && nY >= drlgCoords.nSubtileY + 1 && nX < drlgCoords.nSubtileX + drlgCoords.nSubtileWidth - 1 && nY < drlgCoords.nSubtileY + drlgCoords.nSubtileHeight - 1 && !COLLISION_CheckMaskWithSizeXY(pRoom, nX, nY, nSizeX + 6, nSizeY + 6, COLLIDE_MASK_PLACEMENT)) {
			D2_ASSERT(pRoom);
	...
	for (int32_t i = 0; i < 1; ++i) {
		...
		if (drlgCoords.nSubtileWidth >= 2 && drlgCoords.nSubtileHeight >= 2 && drlgCoords.nSubtileWidth > nSizeX && drlgCoords.nSubtileHeight > nSizeY) {
			for (int32_t j = 0; j < 5; ++j) {
				...
				if (nX && nY && (uint16_t)nX >= drlgCoords.nSubtileX + 1 && (uint16_t)nY >= drlgCoords.nSubtileY + 1 && (uint16_t)nX < drlgCoords.nSubtileX + drlgCoords.nSubtileWidth - 1 && (uint16_t)nY < drlgCoords.nSubtileHeight + drlgCoords.nSubtileY - 1) {
...
D2UnitStrc* __fastcall OBJECTS_PopulateFn4_Barrel(D2GameStrc* pGame, D2ActiveRoomStrc* pRoom, uint8_t nDensity, uint32_t nObjectId, uint16_t nProbability) {
	...
	while (j > 0 && k > 0) {
		...
		if (drlgCoords.nSubtileWidth >= 2 && drlgCoords.nSubtileHeight >= 2 && (uint16_t)nX > drlgCoords.nSubtileX + 2 && (uint16_t)nY > drlgCoords.nSubtileY + 2 && (uint16_t)nX < (int32_t)(drlgCoords.nSubtileWidth + drlgCoords.nSubtileX - pObjectsTxtRecord->dwSizeX) && (uint16_t)nY < (int32_t)(drlgCoords.nSubtileHeight + drlgCoords.nSubtileY - pObjectsTxtRecord->dwSizeY) && !COLLISION_CheckMaskWithSizeXY(pRoom, nX, nY, pObjectsTxtRecord->dwSizeX + 2, pObjectsTxtRecord->dwSizeY + 2, COLLIDE_MASK_PLACEMENT)) {
			D2_ASSERT(pRoom);
			...
			while (!ITEMS_RollLimitedRandomNumber(&pObjectControl->pSeed, nMax / 2) && bSpawnObject) {
				...
				for (int32_t i = 0; !bSpawnObject && i < 15; ++i) {
					...
					bSpawnObject = drlgCoords.nSubtileWidth >= 2 && drlgCoords.nSubtileHeight >= 2 && (uint16_t)nX > drlgCoords.nSubtileX + 2 && (uint16_t)nY > drlgCoords.nSubtileY + 2 && (uint16_t)nX < (int32_t)(drlgCoords.nSubtileWidth + drlgCoords.nSubtileX - pObjectsTxtRecord->dwSizeX) && (uint16_t)nY < (int32_t)(drlgCoords.nSubtileHeight + drlgCoords.nSubtileY - pObjectsTxtRecord->dwSizeY) && !COLLISION_CheckMaskWithSizeXY(pRoom, nX, nY, pObjectsTxtRecord->dwSizeX + 2, pObjectsTxtRecord->dwSizeY + 2, COLLIDE_MASK_PLACEMENT);
				}
...
D2UnitStrc* __fastcall OBJECTS_PopulateFn5_Crate(D2GameStrc* pGame, D2ActiveRoomStrc* pRoom, uint8_t nDensity, uint32_t nObjectId, uint16_t nProbability) {
	...
	while (j > 0 && k > 0) {
		...
		if (drlgCoords.nSubtileWidth >= 2 && drlgCoords.nSubtileHeight >= 2 && (uint16_t)nX > drlgCoords.nSubtileX + 2 && (uint16_t)nY > drlgCoords.nSubtileY + 2 && (uint16_t)nX < (int32_t)(drlgCoords.nSubtileWidth + drlgCoords.nSubtileX - pObjectsTxtRecord->dwSizeX) && (uint16_t)nY < (int32_t)(drlgCoords.nSubtileHeight + drlgCoords.nSubtileY - pObjectsTxtRecord->dwSizeY)) {
	...
	for (uint16_t nX = pDrlgCoords->nSubtileX; nX < nXEnd; nX += 2) {
		for (uint16_t nY = pDrlgCoords->nSubtileY; nY < nYEnd; nY += 2) {
			...
			if (pDrlgCoords->nSubtileWidth >= 2 && pDrlgCoords->nSubtileHeight >= 2 && nX > pDrlgCoords->nSubtileX + 2 && nY > pDrlgCoords->nSubtileY + 2) {
				if (nX < (int32_t)(pDrlgCoords->nSubtileWidth + pDrlgCoords->nSubtileX - pObjectsTxtRecord->dwSizeX) && nY < (int32_t)(pDrlgCoords->nSubtileHeight + pDrlgCoords->nSubtileY - pObjectsTxtRecord->dwSizeY)) {
					if (!COLLISION_CheckMaskWithSizeXY(pRoom, nX, nY, pObjectsTxtRecord->dwSizeX + 2, pObjectsTxtRecord->dwSizeY + 2, COLLIDE_MASK_PLACEMENT)) {
						D2_ASSERT(pRoom);
...
int32_t __fastcall OBJECTS_SpawnNothing2(D2GameStrc* pGame, D2ActiveRoomStrc* pRoom, D2DrlgCoordsStrc* pDrlgCoords) {
	...
	for (uint16_t nX = pDrlgCoords->nSubtileX + 1; nX < nXEnd; nX += 2) {
		for (uint16_t nY = pDrlgCoords->nSubtileY + 1; nY < nYEnd; nY += 2) {
			...
			if (pDrlgCoords->nSubtileWidth >= 2 && pDrlgCoords->nSubtileHeight >= 2 && nX > pDrlgCoords->nSubtileX + 2 && nY > pDrlgCoords->nSubtileY + 2 && nX < pDrlgCoords->nSubtileX + pDrlgCoords->nSubtileWidth - 1 && nY < pDrlgCoords->nSubtileY + pDrlgCoords->nSubtileHeight - 1 && !COLLISION_CheckMaskWithSizeXY(pRoom, nX, nY, 3u, 3u, COLLIDE_MASK_PLACEMENT)) {
...
void __fastcall OBJECTS_SpawnArmorStandOrWeaponRack(D2GameStrc* pGame, D2ActiveRoomStrc* pRoom, D2DrlgCoordsStrc* pDrlgCoords, int32_t bWeaponRack) {
	...
	if (nRand) {
		...
		for (uint16_t nY = pDrlgCoords->nSubtileY + pObjectsTxtRecord->dwSizeY; nY < nYEnd; nY += 2 * pObjectsTxtRecord->dwSizeY) {
			if (pDrlgCoords->nSubtileWidth >= (int32_t)(pObjectsTxtRecord->dwSizeX + 2) && pDrlgCoords->nSubtileHeight >= (int32_t)(pObjectsTxtRecord->dwSizeY + 2) && nX > pDrlgCoords->nSubtileX + 1 && nY > pDrlgCoords->nSubtileY + 1 && nX < (int32_t)(pDrlgCoords->nSubtileX - pObjectsTxtRecord->dwSizeX + pDrlgCoords->nSubtileWidth - 2) && nY < (int32_t)(pDrlgCoords->nSubtileY - pObjectsTxtRecord->dwSizeY + pDrlgCoords->nSubtileHeight - 2)) {
