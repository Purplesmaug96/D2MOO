...
void __fastcall SUNITDMG_FillDamageValues(D2GameStrc* pGame, D2UnitStrc* pAttacker, D2UnitStrc* pDefender, D2DamageStrc* pDamage, int32_t a5, uint8_t nSrcDam) {
	...
	if (!(pDamage->dwHitFlags & DAMAGEHITFLAG_1)) {
		...
		if (!a5 && (nWeaponMastery = SKILLS_GetWeaponMasteryBonus(pAttacker, sub_6FC7C7B0(pAttacker), 0, 2), nWeaponMastery > 0) // NOLINT(bugprone-assignment-in-if-condition)
...
int32_t __fastcall SUNITDMG_ApplyDodge(D2UnitStrc* pAttacker, D2UnitStrc* pDefender, int32_t bAvoid) {
	if (pDefender) {
		if (pDefender->dwUnitType == UNIT_PLAYER && (pDefender->dwAnimMode == PLRMODE_RUN || pDefender->dwAnimMode == PLRMODE_WALK) || pDefender->dwUnitType == UNIT_MONSTER && (pDefender->dwAnimMode == MONMODE_RUN || pDefender->dwAnimMode == MONMODE_WALK)) {
