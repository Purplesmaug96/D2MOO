...
void __fastcall SUNIT_WarpPlayer(D2GameStrc* pGame, D2UnitStrc* pTarget, D2UnitStrc* pRoomTile) {
	...
	if ((nDestinationLevel == LEVEL_DURIELSLAIR || nDestinationLevel == LEVEL_THEWORLDSTONEKEEPLEV1 || nDestinationLevel == LEVEL_THEWORLDSTONECHAMBER || nDestinationLevel == LEVEL_GLACIALCAVESLEV1 || nDestinationLevel == LEVEL_DURANCEOFHATELEV1) && QUESTS_LevelWarpCheck(pGame, pTarget, nSourceLevel, nDestinationLevel)) {
		return;
	}
	...
	for (D2PresetUnitStrc* i = pPresetUnit; i; i = i->pNext) {
		if (i->nUnitType != UNIT_MONSTER && !(i->bSpawned & 1)) {
			if (nLevelId == LEVEL_PANDEMONIUMFINALE) {
				if (i->nUnitType != UNIT_OBJECT || (i->nIndex != OBJECT_WIRTSBODY && i->nIndex != OBJECT_GOLD_PLACEHOLDER && i->nIndex != OBJECT_CAINGIBBET)) {
