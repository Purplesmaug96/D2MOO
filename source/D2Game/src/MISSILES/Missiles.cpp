...
D2UnitStrc* __fastcall MISSILES_CreateMissileFromParams(D2GameStrc* pGame, D2MissileStrc* missileParams) {
	...
	if (nVelocity) {
		if (pTarget) {
			if (pTarget != missileParams->pOwner && CLIENTS_GetUnitX(pTarget) == CLIENTS_GetUnitX(missileParams->pOwner) && CLIENTS_GetUnitY(pTarget) == CLIENTS_GetUnitY(missileParams->pOwner)) {
				pTarget = nullptr;
				++nTargetX;
				++nTargetY;
			}
