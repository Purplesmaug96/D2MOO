...
BOOL __fastcall D2Common_SKILLMANA_CheckStartStat_6FDB1400(D2UnitStrc* pUnit, D2SkillStrc* pSkill) {
	if (!pUnit || pUnit->dwUnitType != UNIT_PLAYER || pSkill->pSkillsTxt->wStartMana <= 0 || STATLIST_UnitGetStatValue(pUnit, STAT_MANA, 0) >= (int)pSkill->pSkillsTxt->wStartMana << 8 || STATES_CheckState(pUnit, STATE_INFERNO)) {
		return TRUE;
	}
...
int __stdcall D2Common_11024(D2UnitStrc* pUnit, D2UnitStrc* pItem, D2SkillStrc* pSkill, int nType, BOOL* pHasThrowBonus) {
	...
	if (!ITEMS_CheckIfThrowable(pItem) || !pSkill->pSkillsTxt || pSkill->pSkillsTxt->nITypeA[0] <= 0 || !ITEMS_CheckType(pSkill->pSkillsTxt->nITypeA[0], ITEMTYPE_THROWN_WEAPON) || pSkill->pSkillsTxt->nRange != 2) {
		return 0;
	}
