...
D2DrlgRoomStrc* __fastcall DRLGMAZE_GetFreeLocationForRoomNorth(D2DrlgLevelStrc* pLevel) {
	...
	for (D2DrlgRoomStrc* pDrlgRoom = pLevel->pFirstRoomEx; pDrlgRoom; pDrlgRoom = pDrlgRoom->pDrlgRoomNext) {
		if ((!pResult || pDrlgRoom->nTileYPos > pResult->nTileYPos) && DRLGMAZE_CheckIfMayPlaceAdjacentPresetRoom(pDrlgRoom, ALTDIR_SOUTH)) {
			pResult = pDrlgRoom;
		}
	}
	return pResult;
...
D2DrlgRoomStrc* __fastcall DRLGMAZE_GetFreeLocationForRoomSouth(D2DrlgLevelStrc* pLevel) {
	...
	for (D2DrlgRoomStrc* pDrlgRoom = pLevel->pFirstRoomEx; pDrlgRoom; pDrlgRoom = pDrlgRoom->pDrlgRoomNext) {
		if ((!pResult || pDrlgRoom->nTileYPos < pResult->nTileYPos) && DRLGMAZE_CheckIfMayPlaceAdjacentPresetRoom(pDrlgRoom, ALTDIR_NORTH)) {
			pResult = pDrlgRoom;
		}
	}
	return pResult;
