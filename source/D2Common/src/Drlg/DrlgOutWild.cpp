...
void __fastcall DRLGOUTWILD_GetBridgeCoords(D2DrlgLevelStrc* pLevel, int* pX, int* pY) {
	...
	for (int nY = 1; nY < pLevel->pOutdoors->nGridWidth - 1; ++nY) {
		if (DRLGGRID_GetGridEntry(&pLevel->pOutdoors->pGrid[0], nX, nY) == 28 && DRLGOUTDOORS_GetPackedGrid2Info(pLevel->pOutdoors, nX, nY).nPickedFile == 1) {
			*pX = nX;
			*pY = nY;
			return;
		}
...
void __fastcall DRLGOUTWILD_InitAct1OutdoorLevel(D2DrlgLevelStrc* pLevel) {
	...
	if (pLevel->nLevelId != LEVEL_BLOODMOOR && pLevel->nLevelId != LEVEL_COLDPLAINS && pLevel->nLevelId != LEVEL_BURIALGROUNDS) {
		...
		do {
			...
			if (pVertex->nPosX < pNextVertex->nPosX && pPreviousVertex->nPosY > pVertex->nPosY && !(pVertex->dwFlags & 1) && !(pPreviousVertex->dwFlags & 1) || pVertex->nPosY > pNextVertex->nPosY && pPreviousVertex->nPosX > pVertex->nPosX && !(pVertex->dwFlags & 1) && !(pPreviousVertex->dwFlags & 1)) {
...
BOOL __fastcall DRLGOUTWILD_TestSpawnRiver(D2DrlgLevelStrc* pLevel, int nX) {
	for (int nY = 0; nY < pLevel->pOutdoors->nGridHeight; ++nY) {
		if (DRLGOUTDOORS_GetPackedGrid2Info(pLevel->pOutdoors, nX, nY).bHasDirection || DRLGOUTDOORS_GetPackedGrid2Info(pLevel->pOutdoors, nX + 1, nY).bHasDirection) {
			return FALSE;
		}
	}
	...
	if (pLevel->nLevelId != LEVEL_MOOMOOFARM) {
		if (pLevel->pOutdoors->dwFlags & 0x10) {
			...
			if (pLevel->pOutdoors->nGridHeight <= 0) {
				...
				while (!DRLGOUTDOORS_GetPackedGrid2Info(pLevel->pOutdoors, nX, nY).bHasDirection && !DRLGOUTDOORS_GetPackedGrid2Info(pLevel->pOutdoors, nX + 1, nY).bHasDirection) {
					++nY;
