...
void __fastcall DRLGPRESET_SpawnHardcodedPresetUnits(D2DrlgRoomStrc* pDrlgRoom) {
	...
	if (pMazeMap->bInited == 1) {
		...
		if ((nLevelPrestId >= LVLPREST_ACT1_WILD_BORDER_1 && nLevelPrestId <= LVLPREST_ACT1_WILD_BORDER_4) && (pDrlgRoom->pLevel->nLevelId == LEVEL_BLOODMOOR && pMazeMap->nPickedFile == 3)) {
	...
	if (pDrlgRoom->pLevel->nLevelId == LEVEL_BURIALGROUNDS && !pMaze->pTombStoneTiles) {
		...
		for (int i = 0; i < pDrlgRoom->nTileHeight; ++i) {
			for (int j = 0; j < pDrlgRoom->nTileWidth; ++j) {
				...
				if (nTileInformation.nTileStyle == 10 && nTileInformation.nTileSequence >= 23u && nTileInformation.nTileSequence <= 27u) {
