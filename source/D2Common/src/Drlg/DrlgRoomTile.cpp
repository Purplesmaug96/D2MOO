		...
		for (int nTileOffsetX = 0; nTileOffsetX < nTileCountX; ++nTileOffsetX) {
			...
			if (nTileInformation.bHidden) {
				...
				case TILETYPE_WALL_RIGHT_DOOR: {
					...
					if (nLevelId == LEVEL_ID_ACT5_BARRICADE_1 || nLevelId == LEVEL_ARREATPLATEAU || nLevelId == LEVEL_TUNDRAWASTELANDS) {
			...
			if (!bTileAdded) {
				...
				if (nTileInformation.bIsWall) {
					...
					if ((nTileType == TILETYPE_WALL_RIGHT_EXIT || nTileType == TILETYPE_WALL_LEFT_EXIT) && pDrlgRoom->pLevel->nLevelId != LEVEL_PANDEMONIUMRUN1)
#else
	...
	if (D2RoomTileStrc* pWarpTile = DRLGROOMTILE_FindDestinationWarpTile(pDrlgRoom, nTileInformation)) {
		...
		if (pWarpDef->dwLitVersion) {
			...
			for (int i = 0; i < pDrlgRoom->pTileGrid->nFloors; ++i) {
				...
				if (D2CMP_10078_GetTileStyle(pFloorTileData->pTile) == nTileInformation.nTileSequence && (unsigned int)D2CMP_10082_GetTileSequence(pFloorTileData->pTile) < 4) {
