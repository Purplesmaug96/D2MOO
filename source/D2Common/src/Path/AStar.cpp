...
BOOL PATH_AStar_TargetLocationHasEnoughRoom(D2PathInfoStrc* pPathInfo) {
	...
	return !COLLISION_CheckAnyCollisionWithPattern(pStartRoom, nTargetX - 2, nTargetY - 2, nCollisionPattern, nCollisionMask) || !COLLISION_CheckAnyCollisionWithPattern(pStartRoom, nTargetX - 2, nTargetY + 2, nCollisionPattern, nCollisionMask) || !COLLISION_CheckAnyCollisionWithPattern(pStartRoom, nTargetX + 2, nTargetY - 2, nCollisionPattern, nCollisionMask) || !COLLISION_CheckAnyCollisionWithPattern(pStartRoom, nTargetX + 2, nTargetY + 2, nCollisionPattern, nCollisionMask) || !COLLISION_CheckAnyCollisionWithPattern(pStartRoom, nTargetX - 2, nTargetY + 0, nCollisionPattern, nCollisionMask) || !COLLISION_CheckAnyCollisionWithPattern(pStartRoom, nTargetX + 0, nTargetY - 2, nCollisionPattern, nCollisionMask) || !COLLISION_CheckAnyCollisionWithPattern(pStartRoom, nTargetX + 2, nTargetY + 0, nCollisionPattern, nCollisionMask) || !COLLISION_CheckAnyCollisionWithPattern(pStartRoom, nTargetX + 0, nTargetY + 2, nCollisionPattern, nCollisionMask);
}
...
int __fastcall PATH_AStar_ComputePath(D2PathInfoStrc* pPathInfo) {
	...
	for (D2PathFoWallNodeStrc* pCurNodeToVisit = PATH_AStar_PopBestScoreForVisit(&tContext);
		...
		if (pBestNode == nullptr || pCurNodeToVisit->nHeuristicDistanceToTarget < pBestNode->nHeuristicDistanceToTarget || (pCurNodeToVisit->nHeuristicDistanceToTarget == pBestNode->nHeuristicDistanceToTarget && pCurNodeToVisit->nBestDistanceFromStart > (pBestNode->nBestDistanceFromStart + 5))) {
