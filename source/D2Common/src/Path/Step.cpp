...
BOOL __fastcall sub_6FDAD5E0(D2DynamicPathStrc* pDynamicPath, D2ActiveRoomStrc* pDestRoom, D2PathPointStrc tDest) {
	...
	if ((pDynamicPath->dwFlags & PATH_MISSILE_MASK) == 0 || COLLISION_GetRoomBySubTileCoordinates(pDynamicPath->pRoom, PATH_FromFP16(dwPrecisionSubileCenterX), PATH_FromFP16(dwPrecisionSubtileCenterY))) {
...
void PATH_RecacheRoomAtCoordIfNeeded(D2DynamicPathStrc* pPath, D2ActiveRoomStrc* pHintRoom, uint32_t nPrecisionX, uint32_t nPrecisionY) {
	if ((pPath->dwFlags & PATH_MISSILE_MASK) == 0 || COLLISION_GetRoomBySubTileCoordinates(pPath->pRoom, PATH_FromFP16(nPrecisionX), PATH_FromFP16(nPrecisionY))) {
