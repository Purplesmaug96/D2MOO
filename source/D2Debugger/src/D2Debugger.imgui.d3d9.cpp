...
void GetWindowPositionFromGameWindow(int& x, int& y) {
	...
	if (windowRect.left == monitorInfo.rcMonitor.left && windowRect.right == monitorInfo.rcMonitor.right && windowRect.top == monitorInfo.rcMonitor.top && windowRect.bottom == monitorInfo.rcMonitor.bottom) {
...
void CleanupDeviceD3D() {
	if (gD2DebuggerData.pd3dDevice) {
		gD2DebuggerData.pd3dDevice->Release();
		gD2DebuggerData.pd3dDevice = nullptr;
	}
	if (gD2DebuggerData.pD3D) {
		gD2DebuggerData.pD3D->Release();
		gD2DebuggerData.pD3D = nullptr;
	}
}
